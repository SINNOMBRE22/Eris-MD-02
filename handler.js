...
if (!universalWords.includes(firstWord) && this?.user?.jid !== chat.primaryBot) return
    ...
    try {
        ...
        if (sender.endsWith('@lid')) {
            if (m.isGroup) return // fantasma de grupo sin mapeo -> ignorar
            // Privado: intentar resolver el número real; si no, usar el LID como identificador
            const alt = m.key?.remoteJidAlt || m.key?.participantAlt || m.key?.senderPn
            if (alt && !alt.endsWith('@lid')) sender = alt
        }
